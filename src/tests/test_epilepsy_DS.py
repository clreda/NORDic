...
#file_folder="refractory_epilepsy2/" ## targets MP (default:+1)
#file_folder="refractory_epilepsy2sigs/" ## signatures MP
file_folder="refractory_epilepsy2targets-1/" ## targets MP (default:-1)
...
drug_names = list(pd.read_csv("Refractory_Epilepsy_Data/scores.csv", index_col=0)["drug_name"])
#target_args = {
#    "DrugBank": {
...
#drug_ids = list(pd.read_csv("Refractory_Epilepsy_Data/scores.csv", index_col=0).index)
#targets.columns = drug_ids
...
signatures_gen = pd.read_csv(signature_fname, index_col=0)
...
## Restrict to 34 antiepileptic drugs and proconvulsants
...
cols = [ground_truth_scores_convert_names.loc[int(i)]["drug_name"] for i in signatures_gen.columns]
...
scores_fname = file_folder+"treatments_scores_targets.csv"
...
if (scores_.shape[0]==0):
    ...
    scores_all.index = [ground_truth_scores_convert_names.loc[int(i)]["drug_name"] for i in scores_all.index]
    scores_ = scores.join(ground_truth_scores, how="inner")
scores_ = scores_.sort_values(by="Simulator Score", ascending=False)
print(scores_.head())
print(scores_all.head())
...
true = ground_truth_scores.join(scores, how="outer")
try:
    true = true.loc[signatures_gen.columns]
except:
    signatures_gen.columns = [ground_truth_scores_convert_names.loc[int(i)]["drug_name"] for i in signatures_gen.columns]
    true = true.loc[signatures_gen.columns]
scores_ = scores_.loc[signatures_gen.columns]
scores = scores.loc[signatures_gen.columns]
scores_all = scores_all.loc[signatures_gen.columns]
...
## Heatmap
true = np.ravel(true[ground_truth_scores.columns[0]].fillna(0).astype(int).values)
print("%d elements" % true.shape[0])
